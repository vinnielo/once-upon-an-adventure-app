...
class TextBox extends React.Component {
  ...
  componentDidMount() {
    ...
    const getUserData = async () => {
      ...
      const { name, sprite, homeFirst, lives, money } = user.sprite[0];
      ...
      if (homeFirst === true) {
        this.setState({
          storyString:
            userName +
            " awakens in a strange land. " +
            userName +
            " looks around. There's a beautiful field and there are paths that seem to lead in different directions. There seems to be a path that leads to a forest, and one that leads to a cliff. In the distance, " +
            userName +
            " can see a beautiful castle - it looks like it would be hard to take in battle! ",
        });
      ...
      if (homeFirst === false) {
        this.setState({
          storyString:
            userName +
            " enters where they first came into the land. " +
            userName +
            " looks around and decides what to do. ",
        });
      }
    };
  ...
  poofGoes = () => {
    ...
    this.updateStory(
      "And then, in another POOF, he was gone! " +
        userName +
        " looked around again and thought about what to do... "
    );
    this.createStory();
  };
  ...
  updateStory = (storyObjectPath) => {
    this.setState({
      storyString: this.state.storyString + " " + storyObjectPath,
    });
  };
  ...
  createStory = () => {
    ...
    API.createStory(postObj, id)
      .then((res) => console.log("story created"))
  ...
  render() {
    return (
      <div>
        ...
        <Inventory money={this.state.money} />
        ...
        <div className="card-body text-center">
          ...
          <button
            ...
            onClick={this.findGameLog}
            data-toggle="modal"
            data-target="#game-log"
          >
            Game Log!
          </button>
        </div>
        ...
        <div className={this.state.poofShow}>
          <img
            ...
            src={require("../../images/prettypoof.gif")}
          />
        </div>

        <div className={this.state.wilsonShow}>
          <img
            className="appear-image"
            src={require("../../assets/sprites/wilson.gif")}
          />
        </div>
        <div>
          <img
            className="avatar-image"
            src={require("../../assets/sprites/1-" + this.state.image)}
          />
        </div>
          ...
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="scroll-log">{this.state.gameLog}</div>
              </div>
